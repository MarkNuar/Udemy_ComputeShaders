...
public class GrassClumps : MonoBehaviour
{
    ...
    void InitShader()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            pos = transform.TransformPoint(pos); // from local to world space 
        ...
        argsArray[0] = (uint)mesh.GetIndexCount(0);
        argsArray[1] = (uint)count;

        argsBuffer = new ComputeBuffer(1, 5 * sizeof(uint), ComputeBufferType.IndirectArguments); // when setting the arg buffer, specify the third argument!
    ...
    void Update()
    {
        ...
        Graphics.DrawMeshInstancedIndirect(mesh, 0, material, bounds, argsBuffer); // render the grass clumps
    }
